...
/**
 * BookCard (recommendations panel) — tracks:
 *  - flip_book_card  (front → back, i.e. "click to see details")
 *  - save_book
 *  - pass_book
 */
const BookCard = ({ book, onSave, onReject, isSaved, userId }) => {
    ...
    const handleFlip = () => {
        if (!isFlipped) {
            // Only log the first flip (front → back = user viewed details)
            if (userId) {
    ...
    return (
        <div className="recommendations-flip-card-wrapper">
            <div className={`recommendations-flip-card-inner ${isFlipped ? 'flipped' : ''}`}>
                {/* Front */}
                <div className="recommendations-flip-card-front" onClick={handleFlip}>
                    {hasFilterMatch && (
